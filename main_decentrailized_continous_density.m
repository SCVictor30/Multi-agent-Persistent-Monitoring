...
tar = 1:20;
A0= zeros(1,L+1);A0(tar+1) = 0.1; %increasing rate
R0=zeros(1,L+1);R0(tar+1) = 4; %uncertainty
% agent param
rs= 2;
rc = 2*rs;
ri = 2*rs;
...
for j = 1:numel(s)
    ...
    for ii = 1:L+1
        if A0(ii) ~= 0
            plot(0:T,(ii-1)*ones(1,length(0:T)),'k--','LineWidth',1);
        end
    end
...
title(' trajectory respect to time','Interpreter','Latex');
xlabel('Time t','Interpreter','Latex');
ylabel('s','Interpreter','Latex');

figure(3)
plot(t,J1_list,'r-','LineWidth',1);
title('Cost respect to time','Interpreter','Latex');
xlabel('Time t','Interpreter','Latex');
ylabel('J cost','Interpreter','Latex');